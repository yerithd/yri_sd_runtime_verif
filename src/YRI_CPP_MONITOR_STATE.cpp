...
const QString YRI_CPP_MONITOR_STATE::_MONITOR_STATE_NAME_string_key("MONITOR_STATE_NAME");


void YRI_CPP_MONITOR_STATE::
							...
							QString db_query_TABLE__db_query_COLUMN)
{
	if (is_FINAL_STATE())
	{
		QDEBUG_STRING_OUTPUT_2("YRI_CPP_MONITOR_STATE::set_PRE_CONDITION_notIN",
                             ...
                             QString db_query_TABLE__db_query_COLUMN)
{
	if (is_START_STATE())
	{
		QDEBUG_STRING_OUTPUT_2("YRI_CPP_MONITOR_STATE::set_POST_CONDITION_notIN",
...
QString YRI_CPP_MONITOR_STATE::GET_STRING_representation_OF_state_condition()
{
	QString result;
...
QString YRI_CPP_MONITOR_STATE::GET_STRING_representation_OF_state_condition__ENGINEERING_ACRONYM()
{
	QString result;
...
const QStringList &YRI_CPP_MONITOR_STATE::
...
void YRI_CPP_MONITOR_STATE::set_START_STATE(YRI_CPP_MONITOR 	&A_RUNTIME_MONITOR,
										   bool 			is_start_state)
{
	if (is_start_state)
	{
		YRI_CPP_MONITOR_EDGE *a_root_edge =
				A_RUNTIME_MONITOR.create_yri_monitor_edge(YRI_CPP_UTILS::EMPTY_STRING,
														 get_MONITOR_STATE_NAME());

		A_RUNTIME_MONITOR.set_yri_root_edge(a_root_edge);
	}
	else
	{
		YRI_CPP_MONITOR_EDGE *a_root_edge = A_RUNTIME_MONITOR.GET_root_edge();

		if (0 != a_root_edge)
		{
			A_RUNTIME_MONITOR.DELETE_yri_monitor_edge(a_root_edge);
		}
	}
...
void YRI_CPP_MONITOR_STATE::set_FINAL_STATE(bool is_final_state)
{
...
void YRI_CPP_MONITOR_STATE::print()
{
