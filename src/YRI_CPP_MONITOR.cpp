...
#include "src/yri-recovery/YRI_CPP_MONITOR_recovery_SQL_INSERT.hpp"


#include "utils/YRI_CPP_MONITOR_ERP-database.hpp"
...
const char YRI_CPP_MONITOR::INCOMING_TRACE_EVENT_SEPARATOR_CHARACTER = ';';



YRI_CPP_MONITOR *YRI_CPP_MONITOR::CREATE_MONITOR(QString RUNTIME_MONITOR_NAME /*= YRI_CPP_UTILS::EMPTY_STRING*/)
{
    YRI_CPP_MONITOR *A_RUNTIME_MONITOR = new YRI_CPP_MONITOR;

    if (0 != A_RUNTIME_MONITOR)
    {
        if (!YRI_CPP_UTILS::isEqualsCaseInsensitive(RUNTIME_MONITOR_NAME,
        ...
        {
            A_RUNTIME_MONITOR
				->set_RUNTIME_MONITOR_NAME(QString("A_YRI_RUNTIME_MONITOR_GENERIC_NAME"));
        }
    }
...
YRI_CPP_MONITOR *YRI_CPP_MONITOR::CREATE_MONITOR(QString db_type,
                                               QString db_name,
...
{
    YRI_CPP_MONITOR *a_new_monitor_FOR_RUNTIME_VERIFICATION =
                    YRI_CPP_MONITOR::CREATE_MONITOR(RUNTIME_MONITOR_NAME);
...
YRI_CPP_MONITOR::~YRI_CPP_MONITOR()
{
    if (0 != _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS)
    {
        _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->close();

        delete _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS;
    }
...
void YRI_CPP_MONITOR::set_yri_root_edge(YRI_CPP_MONITOR_EDGE *ROOT_EDGE)
{
    if (0 != ROOT_EDGE 		&&
    	_ROOT_EDGE != ROOT_EDGE)
    {
        DELETE_yri_monitor_edge(_ROOT_EDGE);
    }
...
void YRI_CPP_MONITOR::
	YRI_register_set_final_state_CALLBACK_FUNCTION
...
bool YRI_CPP_MONITOR::RESET_RUNTIME_MONITOR()
{
	...
	{
		YRI_CPP_MONITOR_STATE *a_state_CURRENT = 0;
...
bool YRI_CPP_MONITOR::CAN_recover_FROM_THIS_ACCEPTING_ERROR_State
             ...
             YRI_CPP_MONITOR_STATE *an_error_accepting_state)
{
        ...
        0 != an_error_accepting_state)
    {
        if (!a_previous_source_state->Get_PRE_CONDITION_notIN().isEmpty())
        {
            ...
            YRI_CPP_MONITOR_recovery_SQL_INSERT
 ...
 */
void YRI_CPP_MONITOR::set_Recovery_action(YRI_CPP_MONITOR_STATE *a_previous_source_state,
                                         YRI_CPP_MONITOR_STATE *an_error_accepting_state)
{
    if (0 == a_previous_source_state ||
        0 == an_error_accepting_state)
 ...
 * STATES DIAGRAMS IN yri_sd_runtime_verif CONTEXT:
 *
 * 1. Each runtime monitor state has only 1 outgoing edge.
 */
bool YRI_CPP_MONITOR::YRI_trigger_an_edge_event(QString 	an_edge_event,
											  bool 		debug_MSG /* = true */)
{
    if (0 == _current_STATE)
    {
    ...
    YRI_CPP_MONITOR_EDGE *cur_STATE_OUTGOING_EDGE = _current_STATE->get_AN_outgoing_edge();
    ...
    bool yri_cur_edge_GUARDED_CONDITION_trigerred =
    		cur_STATE_OUTGOING_EDGE->evaluate_GUARDED_CONDITION_expression();


    if (true == yri_cur_edge_GUARDED_CONDITION_trigerred   &&
    	YRI_CPP_UTILS::isEqualsCaseInsensitive(an_edge_event,
    			cur_STATE_OUTGOING_EDGE->get_EDGE_EVENT_TOKEN()))
    {
    	if (debug_MSG)
    	{
    		...
    		qDebug() << " *[YRI_CPP_MONITOR::YRI_trigger_an_edge_event:] edge event evaluated triggered guarded condition: "
					 << BOOL_TO_STRING(yri_cur_edge_GUARDED_CONDITION_trigerred)
					 << " **";

    		qDebug() << " *[YRI_CPP_MONITOR::YRI_trigger_an_edge_event:] edge event start state: "
    	...
    	if (debug_MSG)
    	{
    		qDebug() << QString(" *[YRI_CPP_MONITOR::YRI_trigger_an_edge_event:] "
    	...
    	if (precondition_IS_TRUE)
    	{
    		...
    		YRI_CPP_MONITOR_STATE * a_potential_accepting_state = cur_STATE_OUTGOING_EDGE->get_TARGET_STATE();

    		if (0 != a_potential_accepting_state 							&&
    			CHECK_whether__STATE__is__Accepting(*a_potential_accepting_state))
    		{

                qDebug() << " *[YRI_CPP_MONITOR::YRI_trigger_an_edge_event:] edge event accepting state: "
                         << a_potential_accepting_state->get_MONITOR_STATE_NAME() << " **";
                qDebug() << " ******************************* ACCEPTING STATE *******************************";

                if (a_potential_accepting_state->is_ERROR_STATE())
    			{
                    qDebug() << " *[YRI_CPP_MONITOR::YRI_trigger_an_edge_event:] EDGE EVENT ERROR STATE: "
                             << a_potential_accepting_state->get_MONITOR_STATE_NAME() << " **";
                    qDebug() << " ******************************* ERROR STATE *******************************";
...
bool YRI_CPP_MONITOR::SET_DB_CONFIGURATION_PARAMETERS(QString db_type,
                                                     QString db_name,
                                                     QString db_ip_address,
                                                     QString db_user_name,
                                                     QString db_user_pwd,
                                                    ...
                                                    )
{
    if (0 == _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS)
    {
        QDEBUG_STRING_OUTPUT_1("!! YRI_CPP_MONITOR::SET_DB_CONFIGURATION_PARAMETERS: "
        					   "QSqlDatabase object instance NOT INSTANTIATED !!!");

        return false;
    }

    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->set_db_type(db_type);

    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->set_db_name(db_name);

    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->set_db_ip_address(db_ip_address);

    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->set_db_user_name(db_user_name);

    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->set_db_user_pwd(db_user_pwd);

    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS
		->set_db_connection_options(db_connection_options);


    bool isDatabaseOpened =
                    _yri_SET_ALGEBRA_inclusion_DATABASE_to_query_for_STATUS->open();

//      QDEBUG_STRING_OUTPUT_2("!! YRI_CPP_MONITOR::SET_DB_CONFIGURATION_PARAMETERS: database opened: ",
 ...
 */
bool YRI_CPP_MONITOR::is_in_SET_ALGEBRA(QString a_SET_VARIABLE,
                                       QString a_SUPPOSED_SET_VARIABLE)
{
//      qDebug() << QString("YRI_CPP_MONITOR::is_in_SET_ALGEBRA; "
//                                              "a_SET_VARIABLE: %1 "
...
//                                               a_SUPPOSED_SET_VARIABLE);
    ...
    int rowCount = YRI_CPP_UTILS::execQueryRowCount(strQuery, true);
...
bool YRI_CPP_MONITOR::
	CHECK_whether__STATE__is__Accepting(YRI_CPP_MONITOR_STATE &A_POTENTIAL_ACCEPTING_STATE)
{
	...
	CHECK_PRE_CONDITION_notIN(QMap<QString, QString> &SET_NOTin_PRE_STATEPROPERTYKEY_TO_VALUE_map)
{
    ...
    for (uint i = 0; i < set_notIN_PRE_KEYS.size(); ++i)
    {
                               ...
                               A_SET_notIN_PRE_PROPERTY_KEY))
        {
			//if (debug_MSG)
			//{
				qDebug() << " *[YRI_CPP_MONITOR::CHECK_PRE_CONDITION_notIN:] precondition_IS_TRUE: True   **";
			//}
...
//                                                               A_SET_notIN_PRE_PROPERTY_value);
    ...
    //if (debug_MSG)
    //{
    	qDebug() << " *[YRI_CPP_MONITOR::CHECK_PRE_CONDITION_notIN:] precondition_IS_TRUE: FALSE   **";
    //}
	...
	CHECK_PRE_CONDITION_IN(QMap<QString, QString> &SET_in_PRE_STATEPROPERTYKEY_TO_VALUE_map)
{
    ...
    for (uint i = 0; i < set_in_KEYS.size(); ++i)
    {
        ...
        if (is_in_SET_ALGEBRA(A_SET_IN_PRE_PROPERTY_value, A_SET_IN_PRE_PROPERTY_KEY))
        {
			//if (debug_MSG)
			//{
				qDebug() << " *[YRI_CPP_MONITOR::CHECK_PRE_CONDITION_IN:] precondition_IS_TRUE: True   **";
			//}
...
//                                                               A_SET_IN_PROPERTY_value);
    ...
    //if (debug_MSG)
    //{
    	qDebug() << " *[YRI_CPP_MONITOR::CHECK_PRE_CONDITION_IN:] precondition_IS_TRUE: FALSE   **";
    //}
//      qDebug() << QString("%1 NOT in set %2")
//                                      .arg(A_SET_IN_PROPERTY_KEY,
//                                               A_SET_IN_PROPERTY_value);
	...
	CHECK_post_condition_notIN(QMap<QString, QString> &SET_notIN_POST_STATEPROPERTYKEY_TO_VALUE_map)
{
    ...
    for (uint i = 0; i < set_notIN_POST_KEYS.size(); ++i)
    {
                               ...
                               A_SET_notIN_POST_PROPERTY_KEY))
        {
			//if (debug_MSG)
			//{
				qDebug() << " *[YRI_CPP_MONITOR::CHECK_post_condition_notIN:] postcondition_IS_TRUE: True   **";
			//}
...
//                                                                                       A_SET_DBnotIN_PROPERTY_value);
    ...
    //if (debug_MSG)
    //{
    	qDebug() << " *[YRI_CPP_MONITOR::CHECK_post_condition_notIN:] postcondition_IS_TRUE: FALSE   **";
    //}
	...
	CHECK_post_condition_IN(QMap<QString, QString> &SET_IN_POST_STATEPROPERTYKEY_TO_VALUE_map)
{
    ...
    for (uint i = 0; i < set_IN_POST_KEYS.size(); ++i)
    {
                              ...
                              A_SET_IN_POST_PROPERTY_KEY))
        {
			//if (debug_MSG)
			//{
				qDebug() << " *[YRI_CPP_MONITOR::CHECK_post_condition_IN:] postcondition_IS_TRUE: True   **";
			//}
...
//                                                               A_SET_dbIN_PROPERTY_value);
    ...
    //if (debug_MSG)
    //{
    	qDebug() << " *[YRI_CPP_MONITOR::CHECK_post_condition_IN:] postcondition_IS_TRUE: FALSE   **";
    //}
//      qDebug() << QString("%1 db NOT in set %2")
//                                      .arg(A_SET_dbIN_PROPERTY_KEY,
//                                               A_SET_dbIN_PROPERTY_value);
...
QString YRI_CPP_MONITOR::YRI_open_HEADER_TEMPLATE_FILE()
{
    QFile a_template_HEADER_FILE_runtime_monitor(FILE_YRI_hpp_TEMPLATE_RUNTIME_MONITOR);
    ...
    {
        QString errMsg("YRI_CPP_MONITOR::YRI_open_HEADER_TEMPLATE_FILE: ");

        errMsg.append(" ne peut pas ouvrir pour lecture le fichier ")
        	  .append(FILE_YRI_hpp_TEMPLATE_RUNTIME_MONITOR)
			  .append(" . ) !\n\n");
    ...
    //      qDebug() << "++ 3. YRI_CPP_UTILS::YRI_HEARDER_hpp_TEMPLATE_RUNTIME_MONITOR: "
    //                       << YRI_CPP_UTILS::YRI_HEARDER_hpp_TEMPLATE_RUNTIME_MONITOR;

    return YRI_CPP_UTILS::YRI_HEARDER_hpp_TEMPLATE_RUNTIME_MONITOR;
}


QString YRI_CPP_MONITOR::YRI_open_SOURCE_TEMPLATE_FILE()
{
    QFile a_template_SOURCE_FILE_runtime_monitor(FILE_YRI_cpp_TEMPLATE_RUNTIME_MONITOR);
    ...
    {
        QString errMsg("YRI_CPP_MONITOR::YRI_open_SOURCE_TEMPLATE_FILE: ");

        errMsg.append(" ne peut pas ouvrir pour lecture le fichier ").
        append(FILE_YRI_cpp_TEMPLATE_RUNTIME_MONITOR).append(" . ) !\n\n");
...
//      qDebug() << "++ 3. YRI_CPP_UTILS::YRI_SOURCE_cpp_TEMPLATE_RUNTIME_MONITOR: "
//                   << YRI_CPP_UTILS::YRI_SOURCE_cpp_TEMPLATE_RUNTIME_MONITOR;

    return YRI_CPP_UTILS::YRI_SOURCE_cpp_TEMPLATE_RUNTIME_MONITOR;
}


QString YRI_CPP_MONITOR::
    YRI_generate_cplusplus_headers_files__AND__SAVE__TO__DISK(QString A_hpp_FILE_NAME,
                                                             QString a_class_NAME /* = "YRI_DB_RUNTIME_VERIF_Main" */)
{
	QString header_FILE_content =
                YRI_generate_cplusplus_headers_files(A_hpp_FILE_NAME,
                                                    a_class_NAME);

    save_yri_SOURCE_FILES(A_hpp_FILE_NAME,
                         header_FILE_content);
...
QString YRI_CPP_MONITOR::
            YRI_generate_cplusplus_headers_files(QString A_hpp_FILE_NAME,
                                                QString a_class_NAME)
{
    ...
    YRI_CPP_MONITOR_EDGE *_AN_EDGE = 0;
    ...
    for (uint i = 0; i < _EDGES.size(); ++i)
    {
        ...
        if (0 != _AN_EDGE && _AN_EDGE != _ROOT_EDGE)
        {
            ...
            EDGE_EVENT_token_with_parenthesis =
                            QString("YRI_SQL_%1()").arg(_AN_EDGE->
            ...
            EXPORTED_EDGE_SOURCE_CODE
				.append(_AN_EDGE->YRI_EXPORT_edge_CLASS_INSTANCE());
        }
    }


    YRI_open_HEADER_TEMPLATE_FILE();


    QString YRI_TO_EXPORT_SOURCE_CODE_HEADER_FILE =
                    YRI_CPP_UTILS::YRI_HEARDER_hpp_TEMPLATE_RUNTIME_MONITOR;


    YRI_TO_EXPORT_SOURCE_CODE_HEADER_FILE
        .replace("_slots_event_declarations_YRI_RTM_REPLACEMENT_QSTRING_",
                 EDGES_EVENTS_TOKENS_declarations);


    YRI_TO_EXPORT_SOURCE_CODE_HEADER_FILE.replace("_YRI_RTM_HEADER_FILE_NAME",
    											 A_hpp_FILE_NAME);


    YRI_TO_EXPORT_SOURCE_CODE_HEADER_FILE.replace("_YRI_RTM_REPLACEMENT_QSTRING",
    											 a_class_NAME);


    return YRI_TO_EXPORT_SOURCE_CODE_HEADER_FILE;
}


QString YRI_CPP_MONITOR::
	GENERATE_METHOD_event_call(QString event_METHOD_name,
                               QString STRING_bool_GUARDED_CONDITION /* = YRI_CPP_UTILS::EMPTY_STRING */)
{
    ...
    if (STRING_bool_GUARDED_CONDITION.isEmpty())
    {
        an_EVENT_METHOD_call
        	.append(QString("YRI_trigger_an_edge_event(\"%1\");")
...
QString YRI_CPP_MONITOR::
	YRI_GENERATE_FINAL_STATE_auto_CODE(QString 			    &a_last_edge_VARIABLE_STRING_pointer,
                                   	  YRI_CPP_MONITOR_EDGE   &_AN_EDGE)
{
    QString return_code;

    YRI_CPP_MONITOR_STATE *a_source_state = _AN_EDGE.get_SOURCE_STATE();
    YRI_CPP_MONITOR_STATE *a_target_state = _AN_EDGE.get_TARGET_STATE();
...
QString YRI_CPP_MONITOR::
	YRI_GENERATE_START_FINAL_STATE_CODE(QString 				&a_last_edge_VARIABLE_STRING_pointer,
                                   	   YRI_CPP_MONITOR_EDGE  &_AN_EDGE)
{
    QString return_code;

    YRI_CPP_MONITOR_STATE *A_STATE = _AN_EDGE.get_SOURCE_STATE();
...
QString YRI_CPP_MONITOR::
        YRI_generate_cplusplus_sources_files__AND__SAVE__TO__DISK(QString A_cpp_FILE_NAME,
                                                                 QString a_class_NAME /* = "YRI_DB_RUNTIME_VERIF_Main" */)
{
	...
	QString SOURCE_FILE_content =
        YRI_generate_cplusplus_sources_files(A_hpp_FILE_NAME,
                                            a_class_NAME);

	save_yri_SOURCE_FILES(A_cpp_FILE_NAME,
						 SOURCE_FILE_content);
...
QString YRI_CPP_MONITOR::
    YRI_generate_cplusplus_sources_files(QString A_hpp_FILE_NAME,
                                        QString a_class_NAME)
{
    ...
    for (uint i = 0; i < _EDGES.size(); ++i)
    {
        ...
        if (0 != _AN_EDGE && _AN_EDGE != _ROOT_EDGE)
        {
            ...
            EVENT_EDGE_DEFINITIONS
				.append(QString("\nYRI_CPP_MONITOR_EDGE *%1 = create_yri_monitor_edge (\"%2\", \"%3\");")
            ...
            EVENT_EDGE_final_states_DEFINITIONS =
                            YRI_GENERATE_START_FINAL_STATE_CODE(last_edge_variable_STRING,
                                                               *_AN_EDGE);
            ...
            FINAL_STATE_AUTO_DEFINITIONS =
                            YRI_GENERATE_FINAL_STATE_auto_CODE(last_edge_variable_STRING,
                                                               *_AN_EDGE);
    ...
    for (uint i = 0; i < _EDGES.size(); ++i)
    {
        ...
        if (0 != _AN_EDGE && _AN_EDGE != _ROOT_EDGE)
        {
            ...
            YRI_CPP_notinset_inset_TRACE_expression *_AN_EDGE_last_guarded_CONDITION_expression =
            		_AN_EDGE->get_guarded_CONDITION_expression();

            if (0 != _AN_EDGE_last_guarded_CONDITION_expression)
            {
                EVENT_EDGE_DEFINITIONS
                	.append(QString("\nYRI_CPP_notinset_inset_TRACE_expression *a_last_edge_%1_GUARDED_CONDITION \n"
                					"\t= new YRI_CPP_notinset_inset_TRACE_expression(%2,\n"
                					"\t\"%3\",\n"
                					"\t\"%4\");\n\n")
            ...
            EVENT_EDGE_DEFINITIONS
            	.append(QString("\nYRI_CPP_MONITOR_EVENT *a_last_edge_event_%1 = "
    ...
    YRI_open_SOURCE_TEMPLATE_FILE();


    QString YRI_TO_EXPORT_SOURCE_CODE_source_FILE =
                    YRI_CPP_UTILS::YRI_SOURCE_cpp_TEMPLATE_RUNTIME_MONITOR;


    YRI_TO_EXPORT_SOURCE_CODE_source_FILE.
    replace("__event_edge_DEFINITIONSYRRTMREPLACEMENTQSTRING__",
            EVENT_EDGE_DEFINITIONS);


    YRI_TO_EXPORT_SOURCE_CODE_source_FILE.
    replace("__slots_event_DEFINITIONS_YRI_RTM_REPLACEMENT_QSTRING__",
            EDGES_EVENTS_TOKENS__DEFINITIONS);
    ...
    YRI_TO_EXPORT_SOURCE_CODE_source_FILE
        .replace("_YRI_RTM_HEADER_FILE_NAME",
                 A_hpp_FILE_NAME);


    YRI_TO_EXPORT_SOURCE_CODE_source_FILE
        .replace("_YRI_RTM_IMPLEMENTATION_FILE_NAME",
                 A_cpp_FILE_NAME);


    YRI_TO_EXPORT_SOURCE_CODE_source_FILE
        .replace("_YRI_RTM_REPLACEMENT_QSTRING",
                 a_class_NAME);



    return YRI_TO_EXPORT_SOURCE_CODE_source_FILE;
}



void YRI_CPP_MONITOR::save_yri_SOURCE_FILES(QString a_header_OR_SOURCE_file_NAME,
...
void YRI_CPP_MONITOR::find_yri_monitor_edges(YRI_CPP_MONITOR_EDGE &an_edge,
                                           QList<YRI_CPP_MONITOR_EDGE *> &resulting_edges)
{
    resulting_edges.clear();

    YRI_CPP_MONITOR_EDGE *cur_edge = 0;
...
void YRI_CPP_MONITOR::find_yri_monitor_edges(QString start_state_key,
                                           ...
                                           QList<YRI_CPP_MONITOR_EDGE *> &resulting_edges)
{
    ...
    for (uint k = 0; k < _EDGES.size(); ++k)
    {
        ...
        if (0 != cur_edge)
        {
            if (YRI_CPP_UTILS::isEqualsCaseInsensitive(cur_edge->get_SOURCE_STATE_KEY(),
                                            		  start_state_key))
            {
                if (YRI_CPP_UTILS::
...
YRI_CPP_MONITOR_EDGE *YRI_CPP_MONITOR::create_yri_monitor_edge(QString source_state_key,
                                                            QString target_state_key)
{
	YRI_CPP_MONITOR_EDGE *A_RESULTING_EDGE = 0;

	QList <YRI_CPP_MONITOR_EDGE *> resulting_CURRENT_edges;

	find_yri_monitor_edges(source_state_key,
						  target_state_key,
    ...
    YRI_CPP_MONITOR_STATE *A_START_STATE =
    		YRI_CPP_MONITOR::create_yri_monitor_state(source_state_key);

    if (0 != A_START_STATE)
    {
        YRI_CPP_MONITOR_STATE *AN_END_STATE =
                        YRI_CPP_MONITOR::create_yri_monitor_state(target_state_key);

        if (0 != AN_END_STATE)
        {
            A_RESULTING_EDGE = new YRI_CPP_MONITOR_EDGE(*A_START_STATE,
            										   *AN_END_STATE);
        ...
        {
            YRI_CPP_MONITOR::DELETE_yri_monitor_state(A_START_STATE->GET_ID());
        }
    }
...
{
    YRI_CPP_MONITOR_EDGE *AN_EDGE =
                    YRI_CPP_MONITOR::create_yri_monitor_edge(source_state_key,
                                                           target_state_key);

    if (0 != AN_EDGE)
    {
        YRI_CPP_MONITOR_EVENT *AN_EVENT = AN_EDGE->set_EDGE_EVENT(event_string);
        ...
        {
            YRI_CPP_MONITOR::DELETE_yri_monitor_edge(AN_EDGE);
        }
    }
...
{
    YRI_CPP_notinset_inset_TRACE_expression *guarded_condition = 0;





    return YRI_CPP_MONITOR::create_yri_monitor_edge(source_state_key,
                                                  target_state_key,
                                                  event_string,
                                                  guarded_condition);
}
...
bool YRI_CPP_MONITOR::DELETE_yri_monitor_edge(YRI_CPP_MONITOR_EDGE *an_edge)
{
    QList<YRI_CPP_MONITOR_EDGE *> resulting_edges;

    if (0 != an_edge)
    {
        find_yri_monitor_edges(an_edge->get_SOURCE_STATE_KEY(),
...
YRI_CPP_MONITOR_STATE *YRI_CPP_MONITOR::find_yri_monitor_state(QString a_state_key)
{
	QList<YRI_CPP_MONITOR_STATE *>::iterator a_state_key_RESULT =
			std::find_if(_STATES.begin(), _STATES.end(), YRI_CPP_MONITOR_STATE_functor(a_state_key));
...
YRI_CPP_MONITOR_STATE *YRI_CPP_MONITOR::
	create_yri_monitor_state(QString a_state_key /* = YRI_CPP_UTILS::EMPTY_STRING */)
{
	YRI_CPP_MONITOR_STATE *a_state_key_RESULT = find_yri_monitor_state(a_state_key);
    ...
    YRI_CPP_MONITOR_STATE *A_NEW_STATE = new YRI_CPP_MONITOR_STATE(a_state_key);
...
bool YRI_CPP_MONITOR::DELETE_yri_monitor_state(int a_state_ID)
{
	bool remove_current_state = false;

	YRI_CPP_MONITOR_STATE *a_cur_state = 0;
...
QString YRI_CPP_MONITOR::generate_in_DOT_format(const QString &digraph_dot_name,
											   bool 		 PRINT_WITH_SQL_EVENT_LOG_TOKEN)
{
    QString DOT_FILE_CONTENT_FOR_USER_OUTPUT;

    YRI_CPP_MONITOR_EDGE *_AN_EDGE = _ROOT_EDGE;
...
QString YRI_CPP_MONITOR::generate_in_DOT_format(bool PRINT_WITH_SQL_EVENT_LOG_TOKEN /* = false */)
{
...
QString YRI_CPP_MONITOR::print_TO_dot_FILE(const QString &program_working_directory,
										  ...
										  bool          YRI_DEBUG_OUTPUT_PARAMETERS /* = true */)
{
    ...
    if (YRI_DEBUG_OUTPUT_PARAMETERS)
    {
        QDEBUG_STRING_OUTPUT_2("*-YRI_CPP_MONITOR::print_TO_dot_FILE; program_executable_args",
                                program_executable_args);
    }


    int res = YRI_CPP_UTILS::start_PROCESS_AND_READ_PROCESS_output_INTO_FILE(DOT_program,
                     	 	 	 	 	 	 	 	 	 	 	 	 	 	program_working_directory,
																			dot_file_name,
																			program_executable_args);

    if (YRI_VIEW_RESULTING_PDF_FILE &&
        res >= 0)
    {
...
void YRI_CPP_MONITOR::set_current_MONITOR_STATE(YRI_CPP_MONITOR_STATE *a_current_STATE)
{
...
void YRI_CPP_MONITOR::set_current_triggered_EDGE(YRI_CPP_MONITOR_EDGE *a_current_triggered_EDGE)
{
